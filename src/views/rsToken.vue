...
const getToken = (defultAccount?: string, defultPassWord?: string, defultRoleId?: string) => {
    ...
    let obj: loginInfo = {
        ...
        scope: type.value == "1"?"rsWebApp":"webApp"
    }
    ...
    axios({
        method: 'post',
        url: type.value == "1" ? 'https://retail.uni-market.com/route/retailauth/oauth/token' : 'https://retaildev.uni-market.com/route/retailauth/oauth/token',
        data: obj,
        transformRequest: [
            function (data) {
                let ret = ''
        ...
        headers: {
            ...
            "Authorization": type.value == "1" ? "Basic cnNXZWJBcHA6NkYycXd1ZHo=" : "Basic d2ViQXBwOjEyMzQ1Ng=="
        },
    }).then(
        (res) => {
            ...
            if (res?.data?.code == "200") {
                if (res?.data?.data?.tokens?.value) {
                    ...
                    if (decoded?.userComGroupVo[0]?.roles && decoded?.userComGroupVo[0]?.roles.length > 0) {
                        ...
                        console.log(syntaxHighlight(obj))
...
<template>
    <div class="getToken">
        ...
        <div class="operate">
            ...
            <el-button @click="getToken('330013110715', '110715','0000')">查看330013110715数据</el-button>
